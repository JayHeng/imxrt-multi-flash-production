...
void LCDIF_DbiSelectArea(LCDIF_Type *base,
                         ...
                         bool isTiled)
{
    uint16_t width  = endX - startX + 1;
    uint16_t height = endY - startY + 1;

    uint16_t vDisplayExtra = isTiled ? 2U : 1U; /* Used for waiting for the data FIFO to be filled */

    base->HDISPLAY0 = LCDIF_HDISPLAY0_DISPLAY_END(width) | LCDIF_HDISPLAY0_TOTAL(width);
    base->HSYNC0    = LCDIF_HSYNC0_START(startX) | LCDIF_HSYNC0_END(endX + 1U);

    base->VDISPLAY0 = LCDIF_VDISPLAY0_DISPLAY_END(height) | LCDIF_VDISPLAY0_TOTAL(height + vDisplayExtra);

    if (isTiled)
    {
        base->VSYNC0 = LCDIF_VSYNC0_START(startY) | LCDIF_VSYNC0_END(endY + 1U);
    }
}
